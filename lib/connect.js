  ...
  job.device.connect(function (error) {
    ...
    job.device.discoverServices([job.service], function(error, services) { // do explicit search for known service
      if (services != undefined && services.length) {
        ...
        log("found service: " + uartService.uuid);
      }
      if (uartService) {
        var characteristic = undefined;
        uartService.discoverCharacteristics([job.characteristic], function(error, characteristics) { // do explicit search for known characteristic
      ...
      } // if uartService - no else, the timeout above tidys up
    }); // job.device.discoverServices()
    // dwk end new
    /* dwk begin comment out old - to be deleted - for some reason discoverAllServicesAndCharacteristics() was very problematic
