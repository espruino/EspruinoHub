...
exports.handlers = {
  ...
  "181d": function (a) { // Xiaomi V1 Scale
    ...
    const isStabilized = status[5];
    const loadRemoved  = status[7];
    const date         = {
    ...
    return {weight: weight.toFixed(2), unit, isStabilized, loadRemoved, ...date};
  },
  "fe95": function (d) {
  ...
  "fee0": function (d) {
    return {steps: (0xff & d[0] | (0xff & d[1]) << 8)};
  },
