...
var connectTimer = setTimeout(function() {
  ...
  console.log("MQTT NOT CONNECTED AFTER 10 SECONDS");
}, 10000);
  ...
  if (path[0]=="ble" && path[1]=="read") {
    ...
    if (discovery.inRange[id]) {
     ...
     connect.read(device, service, charc, function(data) {
      ...
      console.log("Read from "+id+" but not in range");
    }
  }
  if (path[0]=="ble" && path[1]=="nus") {
    var id = config.deviceToAddr(path[2]);
    if (discovery.inRange[id]) {
      //if (path[3]=="connect")
    } else
      console.log("NUS request for "+id+" but not in range");
  }
});
